...
contract RewardsManager is Ownable {
    ...
    function _distribute() internal {
        ...
        _notifyRewardsAmount(IERC20(rewardToken).balanceOf(address(this)));
    }
    ...
    function transferExtraRewardToken(address _token, address _to) external onlyOperator {
        require(_token != rewardToken, "not extra reward");
    ...
    function _notifyRewardsAmount(uint256 _amount) internal {
        IERC20(rewardToken).safeIncreaseAllowance(staking, _amount);
        IStaxStaking(staking).notifyRewardAmount(_amount);

        emit RewardDistributed(staking, rewardToken, _amount);
    }

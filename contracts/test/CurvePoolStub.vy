...
def __init__(
    ...
    self.name = _name #concat("Curve.fi Factory Plain Pool: ", _name)
    self.symbol = _symbol #concat(_symbol, "-f")
...
def setFactory(_factory: address):
    """
    @notice new stub function added to update factory if we want to test fee withdrawal
